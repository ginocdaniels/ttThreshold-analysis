      ...
      if (angle <= max_angle) {
        ...
        if (update_mother && parton.parents_begin < parton.parents_end &&
          ...
          for (unsigned int p = parton.parents_begin; p < parton.parents_end && p < Particle0.size(); ++p) {
            ...
            // If the parent is a b quark (pdg=5) and the jet flavor is also a b quark, check the next parent
            if (jetFlavors[j] == 5 && (parent_pdg == 5 || parent_pdg==24 || parent_pdg==94) ||(jetFlavors[j] == 21 && parent_pdg == 21)) {
              // Look for the parent of this parent
              const auto& current_parent = mcParticles[parent_idx];
              if (current_parent.parents_begin < current_parent.parents_end && current_parent.parents_begin < Particle0.size()) {
                for (unsigned int gp = current_parent.parents_begin; gp < current_parent.parents_end && gp < Particle0.size(); ++gp) {
                  int grandparent_idx = Particle0[gp];
                  if (grandparent_idx < 0 || grandparent_idx >= mcParticles.size()) continue;
                  int grandparent_pdg = std::abs(mcParticles[grandparent_idx].PDG);
                  // // Checks in place 
                  // if (grandparent_pdg == 5 || grandparent_pdg == 21) continue;
                  if (grandparent_pdg == parent_pdg || grandparent_pdg==0 || parent_pdg==0) continue;
                  
                  
                  // Validate grandparent by checking daughters
                  bool flavor_match_gp = false;
                  const auto& grandparent = mcParticles[grandparent_idx];
                  if (grandparent.daughters_begin < grandparent.daughters_end &&
                      grandparent.daughters_begin < Particle1.size()) {
                    for (unsigned int d = grandparent.daughters_begin; d < grandparent.daughters_end && d < Particle1.size(); ++d) {
                  ...
                  if (flavor_match_gp) {
                    selected_mother_pdg = grandparent_pdg;
                    selected_parent_idx = grandparent_idx;
                    break; // Use the first valid grandparent
                  }
                }
              }
              if (selected_mother_pdg != 0) break; // Exit if we found a valid grandparent
              continue; // If no valid grandparent found, continue to next parent
            }
...
//  std::cout << " p.parents_begin p.parents_end " << p.parents_begin <<  " "  << p.parents_end << std::endl;
    for (unsigned j = p.parents_begin; j != p.parents_end; ++j) {
      ...
      if(pdg_parent == abs(6)) result = pdg_parent;
      else (result=pdg_parent); 
