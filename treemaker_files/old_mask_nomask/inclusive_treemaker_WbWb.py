...
class RDFanalysis:
    ...
    def analysers(df):
        ...
        df= df.Filter("remaining_muons_deltaR <0.5")
        df= df.Filter("remaining_electrons_deltaR <0.5")
        # df = df.Filter(
        #     f"""
        #     bool hasValidJet = false;
        #     for(size_t j = 0; j < jets_R5_theta_unfiltered.size(); j++) {{
        #         bool jetIsValid = true;
        ...
        #             }}
        #         }}
        #         // Check Delta R for electrons if jet is still valid
        #         if(jetIsValid) {{
