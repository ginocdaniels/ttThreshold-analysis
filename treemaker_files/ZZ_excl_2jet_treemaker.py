...
class RDFanalysis:
    ...
    def analysers(df):
        ...
        df=df.Define("Z_collection_1", "FCCAnalyses::MCParticle::sel_pdgID(23,false)(MCParticles)")
        df=df.Define("Z_collection_1_sim_status", "FCCAnalyses::MCParticle::get_simStatus(Z_collection_1)")
        df=df.Define("Z_collection_1_gen_status", "FCCAnalyses::MCParticle::get_genStatus(Z_collection_1)")
        df=df.Define("Z_collection_1_status22", "FCCAnalyses::MCParticle::sel_genStatus(22)(Z_collection_1)")
        df=df.Define("pt_zcollection_1", "FCCAnalyses::MCParticle::get_pt(Z_collection_1_status22)")
        df=df.Define("Z1_22_daughter_test", "FCCAnalyses::myUtils::get_MCDaughter1(Z_collection_1_status22,Particle0)")
        df=df.Define("Z1_22_daughter1", "FCCAnalyses::TruthMatching::get_MCDaughter1(Z_collection_1_status22,Particle0,MCParticles)")
        df=df.Define("Z_daughter_1_pdg", "Z1_22_daughter1[0]")
        df=df.Define("Z_daughter_1_pdg_abs", "abs(Z_daughter_1_pdg)")
        df=df.Define("Z_daughter_2_pdg", "Z1_22_daughter1[1]")
        df=df.Define("Z_daughter_2_pdg_abs", "abs(Z_daughter_2_pdg)")

        df=df.Define("Z_daughter_1_pdg_abs_is_quark", "Z_daughter_1_pdg_abs <= 6 && Z_daughter_1_pdg_abs >= 1")
        df=df.Define("Z_daughter_2_pdg_abs_is_quark", "Z_daughter_2_pdg_abs <= 6 && Z_daughter_2_pdg_abs >= 1")
        df=df.Define("Z_daughter_1_pdg_abs_is_neutrino", "Z_daughter_1_pdg_abs == 12 || Z_daughter_1_pdg_abs == 14 || Z_daughter_1_pdg_abs == 16")
        df=df.Define("Z_daughter_2_pdg_abs_is_neutrino", "Z_daughter_2_pdg_abs == 12 || Z_daughter_2_pdg_abs == 14 || Z_daughter_2_pdg_abs == 16")

        df=df.Filter("(Z_daughter_1_pdg_abs_is_neutrino && Z_daughter_2_pdg_abs_is_quark) || (Z_daughter_1_pdg_abs_is_quark && Z_daughter_2_pdg_abs_is_neutrino)")
        ...
        #     collections_noleps["PFParticles"], deltaR_threshold, 5, "R6"
        # )
        if  saveExclJets:
        ...
        df=df.Define(f"PseudoJetCollection_masked", f"{jetClusteringHelper.jets}[jet_removal_mask]")
        df=df.Define("masked_jet_constitutents", f"{jetClusteringHelper.constituents}[jet_removal_mask]")
        ...
        # without the masked jet collection the pflavor indicies are different then when u call the jetClusteringHelper.jets so it doesnt work but this works perfectly
