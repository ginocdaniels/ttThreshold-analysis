...
def stackPlot(fname,vname,lumi,channel,config,ecm,useLog,showInt,nostack,sel):
    ...
    h_bkg=f_in.Get('x_bkg')#_%s'%channel);
    ...
    hs.Draw("HIST" if not nostack else 'nostack');
...
def cards(mkplots,lumi,xsec_sig,channel,sel,config,bWP,ecm,logy,vname,xtitle,showInt,nostack):
    ...
    h_bkg  = getHist(False,f'p8_ee_WW_ecm{ecm}',vname,"x_bkg",1.0,channel,config,ecm,lumi)
    ...
    h_bkg_btagUp    = getHist(False,f'p8_ee_WW_ecm{ecm}',vname_btagUp,"x_bkg_btagUp",1.0,channel,config,ecm,lumi)
    h_bkg_btagDown  = getHist(False,f'p8_ee_WW_ecm{ecm}',vname_btagDown,"x_bkg_btagDown",1.0,channel,config,ecm,lumi)
    
    h_obs.Add(h_bkg); 
    print('bkg',h_bkg.Integral())
    fout_name=f"rootfiles/{channel}_{sel}_{config}_bWP{bWP}_{ecm}_singleNP.root"
